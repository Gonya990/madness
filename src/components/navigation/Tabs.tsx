...
function normalize(it: TabItem | string): TabItem {
  return typeof it === 'string' ? { value: it, label: it } : it;
}

export function Tabs({ items = [], value, defaultValue, onChange, variant = 'line', className = '', style = {}, ...rest }: TabsProps) {
  ...
  const handle = (v: string) => { if (!isControlled) setInternal(v); onChange && onChange(v); };
