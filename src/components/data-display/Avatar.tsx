...
export function Avatar({ src, name = '', size = 36, shape = 'circle', status, color, className = '', style = {}, ...rest }: AvatarProps) {
  ...
  return (
    <span
      ...
      style={{ width: size, height: size, fontSize: Math.round(size * 0.4), ...(color ? { background: color, color: '#fff' } : {}), ...style }}
