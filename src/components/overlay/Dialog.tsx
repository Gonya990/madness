...
export function Dialog({ open, onClose, title, description, children, footer, size = 'md', closeOnOverlay = true, closeOnEsc = true, className = '', style = {} }: DialogProps) {
  ...
  return (
    <div className="uc-dialog__overlay" onMouseDown={(e) => { if (closeOnOverlay && e.target === e.currentTarget) onClose && onClose(); }}>
      <div role="dialog" aria-modal="true" className={cx('uc-dialog', `uc-dialog--${size}`, className)} style={style}>
