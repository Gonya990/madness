...
export function Button({
  ...
  return (
    <Tag
      ...
      {...rest}
    >
      {loading && <span className="uc-btn__spin" aria-hidden="true" />}
      {!loading && leftIcon && <Icon name={leftIcon} size={iconSize} strokeWidth={2.1} />}
      {children != null && <span>{children}</span>}
      {!loading && rightIcon && <Icon name={rightIcon} size={iconSize} strokeWidth={2.1} />}
    </Tag>
  );
}
